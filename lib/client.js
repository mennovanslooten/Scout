...
(function($) {
	...
	DummyClient.run = function(assert_type, selector, args) {
		var $elts = find(selector);

		if (DummyClient.hasOwnProperty(assert_type)) {
			args.unshift($elts);
			return DummyClient[assert_type].apply(null, args);
		}
	...
	function getCoordinate($elt) {
		...
		// Check the element boundaris pixel by pixel until we find one that
	...
	DummyClient.assertText = function($elts, text) {
		var is_found = false;
	...
	DummyClient.assertExists = function($elts) {
		if ($elts.length > 0) {
			return '';
		}
	...
	DummyClient.assertVisible = function($elts) {
	...
	DummyClient.assertHidden = function($elts) {
		if (getVisible($elts).length === 0) {
			return '';
		}
	...
	DummyClient.assertEmpty = function($elts) {
		...
		return DummyClient.assertLength($elts.children(), 0);
	};


	DummyClient.assertNotEmpty = function($elts) {
		var text = getText($elts);
		...
		return DummyClient.assertMinLength($elts.children(), 1);
	};


	DummyClient.assertLength = function($elts, num) {
		num = parseInt(num, 10);
	...
	DummyClient.assertMinLength = function($elts, num) {
		num = parseInt(num, 10);
		var actual = getVisible($elts).length;
		if (actual >= num) {
			return '';
	...
	DummyClient.assertMaxLength = function($elts, num) {
		num = parseInt(num, 10);
		var actual = getVisible($elts).length;
		if (actual <= num) {
			return '';
	...
	DummyClient.assertValue = function($elts, value) {
		value = value.trim();
	...
	DummyClient.assertHasClass = function($elts, class_name) {
	...
	DummyClient.assertNotHasClass = function($elts, class_name) {
		if (class_name.indexOf('.') !== 0) {
			class_name = '.' + class_name;
		}
	...
	DummyClient.assertDisabled = function($elts) {
	...
	DummyClient.assertInViewport = function($elts) {
	...
	DummyClient.assertNotInViewport = function($elts) {
		var $result = getVisibleInViewport($elts);
	...
	DummyClient.getCoordinate = function($elts) {
		var result = null;
	...
	DummyClient.getCenter = function() {
	...
	DummyClient.getBoundaries = function($elts) {
		var $result = getVisible($elts).first();
	...
	DummyClient.choose = function($elts) {
	...
	DummyClient.getValueOrText = function($elts) {
