...
exports.action_handlers = {
	...
	click: function(selector) {
		...
		if (!is_in_viewport) {
			...
			// console.log('not in viewport:', viewport.scroll_top, ' <= ', center.top, ' :: ',  viewport.scroll_top + viewport.h, ' >= ', center.top, new_scroll_top);
		...
		// console.log('moving mouse to', _mouse.x, _mouse.y, center.left, center.top);
		page.sendEvent('mousemove', _mouse.x, _mouse.y);

		if (_mouse.x !== center.left || _mouse.y !== center.top) {
			//console.log('not yet at', center.left, center.top);
			return false;
		}
