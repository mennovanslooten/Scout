...
exports.reformat = function(_suite) {
...
exports.startTest = function(test_data) {
	...
	log('\n');
	log(bg.darkgray(fg.white('' + new Array(title_width).join('#'))));
	log(bg.darkgray(fg.white('#', test_data.path, new Array(title_width - test_data.path.length - 5).join(' '), '#')));
	log(bg.darkgray(fg.white('' + new Array(title_width).join('#'))));
};
