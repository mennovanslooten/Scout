(function($) {
	...
	function isOpacity0($elt) {
		return parseInt($elt.css('opacity'), 10) === 0;
	}
	...
	function getCoordinate($elt) {
		//console.log('getCoordinate', $elt.text(), $elt.attr('class'));
		...
		//if ($win.height() < top || $win.width() < left) {
			//console.log($win.height(), top, $win.width(), left);
			//console.log('eh?', $elt.text());
			//return offset;
		//}
			...
			for (var y = top; y <= bottom && y >= 0; y += 1) {
				...
				while (elt_at_point) {
					if (elt_at_point === elt) {
						...
						console.log('getCoordinate', $elt.attr('class'), result);
						return result;
					}
		...
		console.log('no pixel found for', $elt.text());
		//return null;
		return offset;
	}
