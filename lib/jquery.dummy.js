(function($) {
	...
	function isCovered($elt) {
		var offset = $elt.offset();

		var left = offset.left;
		var top = offset.top;
		var right = offset.left + $elt.outerWidth();
		var bottom = offset.top + $elt.outerHeight();

		var elt = $elt[0];

		// Check the element pixel by pixel to see if it is (partially) visible
		// Yes, really
		for (var x = left; x <= right; x += 1) {
			for (var y = top; y <= bottom; y += 1) {
				var elt_at_point = document.elementFromPoint(x, y) || elt;
				while (elt_at_point) {
					if (elt_at_point === elt) return false;
