...
(function($) {
	...
	Dummy.run = function(assert_type, selector, args) {
		var $elts = find(selector);

		if (Dummy.hasOwnProperty(assert_type)) {
			args.unshift($elts);
			return Dummy[assert_type].apply(null, args);
		}
	...
	Dummy.assertText = function($elts, text) {
		var is_found = false;
	...
	Dummy.assertExists = function($elts) {
		if ($elts.length > 0) {
			return '';
		}
	...
	Dummy.assertVisible = function($elts) {
	...
	Dummy.assertHidden = function($elts) {
		if (getVisible($elts).length === 0) {
			return '';
		}
	...
	Dummy.assertEmpty = function($elts) {
		...
		return Dummy.assertLength($elts.children(), 0);
	};


	Dummy.assertNotEmpty = function($elts) {
		var text = getText($elts);
		...
		return Dummy.assertMinLength($elts.children(), 1);
	};


	Dummy.assertLength = function($elts, num) {
		num = parseInt(num, 10);
	...
	Dummy.assertMinLength = function($elts, num) {
		num = parseInt(num, 10);
		var actual = getVisible($elts).length;
		if (actual >= num) {
			return '';
	...
	Dummy.assertMaxLength = function($elts, num) {
		num = parseInt(num, 10);
		var actual = getVisible($elts).length;
		if (actual <= num) {
			return '';
	...
	Dummy.assertValue = function($elts, value) {
		value = value.trim();
	...
	Dummy.assertHasClass = function($elts, class_name) {
	...
	Dummy.assertNotHasClass = function($elts, class_name) {
		if (class_name.indexOf('.') !== 0) {
			class_name = '.' + class_name;
		}
	...
	Dummy.assertDisabled = function($elts) {
	...
	Dummy.assertInViewport = function($elts) {
	...
	Dummy.assertNotInViewport = function($elts) {
		var $result = getVisibleInViewport($elts);
	...
	Dummy.getCoordinate = function($elts) {
		var result = null;
	...
	Dummy.getBoundaries = function($elts) {
		var $result = getVisible($elts).first();
	...
	Dummy.getValueOrText = function($elts) {
